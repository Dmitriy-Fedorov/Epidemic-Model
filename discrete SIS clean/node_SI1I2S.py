...
class TransitionDiagram:
    ...
    def __init__(self, paramet):
        # paramet_example = {
        #     'betta_1': 0.1, # infect rate 1
        #     'gamma_1': 0.1, # rec rate 1
        #     'betta_2': 0.1, # infect rate 2
        #     'gamma_2': 0.1, # rec rate 2
        # }
        self.status_list = ['S', 'I1', 'I2']
        self.paramet_list = ['betta_1','gamma_1','betta_2','gamma_2']
        self.transitions = {
            'S': {
                'I1': paramet['betta_1'],
                'I2': paramet['betta_2']
    ...
    def get_next_state(self, G, id):
        ...
        if state == 'S': # edge transition
            ...
            for n_id in sample(list(G[id]), len(G[id])):  # neighbor_id
                ...
                if nstate == 'I1':
                    if uniform() < transition_prob['I1']:
                        next_state = 'I1'
                        break
                elif nstate == 'I2':
                    if uniform() < transition_prob['I2']:
                        next_state = 'I2'
                        break
...
class EpidemicGraph:
    ...
    def __init__(self, N, radius, paramet, I1=1, I2=1, grid_size=[10,10], net='Geometric Random'):
        ...
        self.state_list = ['S', 'I1', 'I2']
        self.create_nodes(N)
        self.init_state(I1, I2)
        self.connect_nodes()
    ...
    def init_state(self, I1, I2):
        unlucky_nodes = sample(range(self.population_size), I1 + I2)
        I1_ = unlucky_nodes[:I1]
        I2_ = unlucky_nodes[I1:]
        for node_id in I1_:
            self.G.node[node_id]['state'] = 'I1'
        for node_id in I2_:
            self.G.node[node_id]['state'] = 'I2'
                
    def load_csv(self, csv, I1, I2):
