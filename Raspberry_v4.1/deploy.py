...
a = ['python3 main_v4.2.py --s {} --f {} --N {N} --delay_koef {delay_koef}\
 --delay {delay} --broker_ip {broker_ip}'.format(i,i+id_step, **FLAGS) for i in range(id_start, id_stop, id_step)]
cmd = str.join(' & \\\n', a)
cmd += ' && fg'
...
'''
python3 deploy.py --s 0 --f 100 --N 100 --delay_koef 0 --step 1
'''